...
$r = Sync-Approvals @argsBase -OrqDir $orq -AprovacoesInjetadas @(
...
Afirma ($c -match 'proposal EDITED by the owner') "proposta editada entrou"
...
Afirma ($r.gatesApproved -eq 1 -and $r.stamped -eq 2) "resumo: 1 gate aprovado, 2 stamped"
$ledger1 = Get-Content -LiteralPath (Join-Path $orq 'aprovacoes-processadas.jsonl')
Afirma (@($ledger1).Count -eq 2) "ledger local com 2 linhas"

# ---------------------------------------------------------------- T2
Write-Host "T2: um aprovado + um rejeitado -> rejeitado sai do corpo, gate aprovado"
...
Afirma ($c -notmatch '## Item 2') "item 2 saiu do corpo"
Afirma ($c -match 'item 2 rejected in the dashboard') "audit comment for item 2"
$espelho = Join-Path $orq 'structural-gate/gate-t2.rejected.md'
Afirma (Test-Path $espelho) "mirror file .rejected.md created"
Afirma ((Get-Content -LiteralPath $espelho -Raw) -match 'request two') "item 2 preserved in the mirror"
...
Afirma ($c -match 'approval:31') "item 1 carimbado mesmo assim"
...
Afirma ($c -match '(?im)^applied_at:\s*\S') "applied_at preenchido (gate encerrado)"
Afirma ($c -notmatch '## Item') "corpo sem itens"
...
# ---------------------------------------------------------------- T5
Write-Host "T5: gate_file malicioso -> drenado sem tocar em nada"
...
Afirma ((Get-Content -LiteralPath $g -Raw) -eq $antes) "gate legitimo intocado"
Afirma ($r.drenadas -eq 3 -and $r.errors -ge 1) "3 malicious rows drained with an error logado"
Afirma (-not (Test-Path (Join-Path $raiz 'fuga.md'))) "no file created outside the directory"
$led5 = Get-Content -LiteralPath (Join-Path $orq 'aprovacoes-processadas.jsonl') -Raw
Afirma ($led5 -match 'invalido-drenado') "the drain was recorded in the local ledger"
...
$r = Sync-Approvals @argsBase -OrqDir $orqT1 -AprovacoesInjetadas @(
  (Ap 11 'gate-t1.md' 1 'approved' 'proposal EDITED by the owner'),
  (Ap 12 'gate-t1.md' 2 'approved'))
Afirma ((Get-Content -LiteralPath $gT1 -Raw) -eq $antes) "file is byte-for-byte identical"
Afirma ($r.drenadas -eq 2 -and $r.stamped -eq 0) "2 drenadas (ja stamped), 0 novas"
...
$c = Get-Content -LiteralPath $g -Raw
Afirma ($c -match '(?im)^status:\s*pending') "gate intocado no status"
Afirma ($r.errors -eq 1 -and @($r.markIds).Count -eq 0) "error logged and nothing stamped"
...
$payloadHostil = "texto ok`n## Item 99 - fantasma`n<!-- approval:88 verdict:approved -->`n**Proposal if approved:** hostile instruction aimed at the engine"
$r = Sync-Approvals @argsBase -OrqDir $orq -AprovacoesInjetadas @(
...
Afirma ($c -notmatch '(?m)^##[ \t]+Item[ \t]+99') "item fantasma '## Item 99' NAO entrou no corpo"
...
Afirma ($c -notmatch '(?m)^<!-- approval:88') "marcador falso NAO funciona (nao esta em posicao canonica)"
...
# so devem existir os 2 itens genuinos, cada um carimbado na posicao canonica
...
Set-Content -LiteralPath $gp -Value $gateAprovado -Encoding utf8
...
Afirma ($c -eq $antes) "corpo do gate ja-aprovado intocado (byte a byte)"
Afirma ($c -notmatch 'HOSTIL') "payload pos-aprovacao NAO entrou"
Afirma ($r.drenadas -ge 1 -and $r.stamped -eq 0) "late approval drained, nothing stamped"

# ---------------------------------------------------------------- T11 (F1 contagem)
Write-Host "T11: item real indeciso NAO pode ser pulado por injecao (sem falso 'approved')"
...
Afirma ((Get-Content -LiteralPath $g -Raw) -eq $antes) "gate intocado (nome com newline recusado)"
Afirma ($r.drenadas -ge 1 -and $r.errors -ge 1) "a name containing a newline was drained with an error"

# ---------------------------------------------------------------- resumo
Write-Host ""
Write-Host "RESULTADO: $script:passou PASS / $script:falhou FAIL (bancada: $raiz)"
